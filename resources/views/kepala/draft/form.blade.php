...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .form-title p { color: #888; font-size: 13px; margin-top: 4px; }
        ...
        /* FILE UPLOAD AREA */
        .field-label { color: #1a8fb3; font-size: 13px; font-weight: 600; margin-bottom: 8px; margin-top: 18px; }
        ...
        .upload-area:hover { border-color: #1a8fb3; background: #eaf6fb; }
        .upload-area.has-file { border-color: #2ecc71; background: #f0fff4; }

        .upload-area input[type=file] {
            position: absolute; inset: 0; opacity: 0; cursor: pointer; width: 100%; height: 100%;
        }
        .upload-icon svg { width: 40px; height: 40px; stroke: #1a8fb3; fill: none; stroke-width: 1.5; margin-bottom: 10px; }
        .upload-text { font-size: 14px; color: #1a8fb3; font-weight: 600; }
        .upload-sub  { font-size: 12px; color: #aaa; margin-top: 4px; }
        .file-chosen { font-size: 13px; color: #27ae60; font-weight: 600; margin-top: 8px; display: none; }
        ...
        .existing-icon {
            width: 36px; height: 36px; background: #e74c3c;
            border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;
        }
        .existing-icon svg { width: 18px; height: 18px; stroke: #fff; fill: none; stroke-width: 2; }
        .existing-name { font-size: 13px; color: #333; font-weight: 500; flex: 1; }
        .existing-link { color: #1a8fb3; font-size: 12px; font-weight: 600; text-decoration: none; }
        .existing-link:hover { text-decoration: underline; }

        .alert-error { background: #ffeaea; color: #c0392b; border-radius: 10px; padding: 10px 14px; font-size: 13px; margin-bottom: 14px; }
...
<div class="shell">
    ...
    <div class="form-card">
        ...
        <div class="info-box">
            ...
            <div class="info-text">
                <div class="name">{{ $pengajuan->pencatatan->nama_lengkap ?? $pengajuan->user->name ?? '-' }}</div>
                <div class="sub">
        ...
        @if($draftSkpp)
            <div class="existing-file">
                ...
                <a href="{{ asset('storage/' . $draftSkpp->file_skpp) }}" target="_blank" class="existing-link">Lihat</a>
            </div>
        @endif

        <form method="POST" action="{{ route('kepala.draft.store', $pengajuan->id) }}" enctype="multipart/form-data">
            @csrf
