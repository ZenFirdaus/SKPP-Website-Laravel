...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-upload { width: 100%; background: #1a8fb3; color: #fff; border: none; border-radius: 50px; padding: 14px; font-size: 15px; font-weight: 700; cursor: pointer; font-family: inherit; display: flex; align-items: center; justify-content: center; gap: 8px; text-decoration: none; transition: background 0.2s, transform 0.1s; }
        ...
        .btn-upload.uploaded { background: #27ae60; }
        .btn-upload.uploaded:hover { background: #219a52; }
...
<div class="shell">
    ...
    <div class="content">
        ...
        {{-- SEARCH & FILTER --}}
        <form method="GET" action="{{ route('kepala.draft.index') }}" class="search-wrap">
            ...
            <div class="filter-row">
                ...
                @if(request('search') || request('sort'))
                    <a href="{{ route('kepala.draft.index') }}" class="filter-clear">✕ Reset</a>
                @endif
            </div>
        </form>
        ...
        @empty
            <div class="empty-state">
                <div class="icon">📄</div>
                <p>{{ request('search') ? 'Tidak ada hasil pencarian.' : 'Belum ada SKPP yang siap diupload.' }}</p>
            </div>
        @endforelse
    </div>
