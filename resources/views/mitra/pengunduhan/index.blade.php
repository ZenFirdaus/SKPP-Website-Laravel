...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .alert-error   { background: #fdecea; color: #c0392b; border-radius: 12px; padding: 12px 16px; font-size: 13px; margin-bottom: 16px; }
        ...
        .inner-avatar { width: 52px; height: 52px; border-radius: 14px; background: rgba(255,255,255,0.25); overflow: hidden; flex-shrink: 0; display: flex; align-items: center; justify-content: center; }
        ...
        .badge-selesai { background: #2ecc71; color: #fff; font-size: 11px; font-weight: 700; padding: 4px 10px; border-radius: 50px; white-space: nowrap; align-self: flex-start; }
        ...
        .btn-unduh { width: 100%; background: #1a8fb3; color: #fff; border: none; border-radius: 50px; padding: 14px; font-size: 15px; font-weight: 700; cursor: pointer; font-family: inherit; display: flex; align-items: center; justify-content: center; gap: 10px; text-decoration: none; transition: background 0.2s, transform 0.1s; }
        .btn-unduh:hover { background: #157a9a; }
        .btn-unduh:active { transform: scale(0.97); }
        .btn-unduh svg { width: 20px; height: 20px; stroke: #fff; fill: none; stroke-width: 2; }

        .empty-state { text-align: center; padding: 80px 24px; }
        .empty-icon { width: 90px; height: 90px; background: rgba(26,143,179,0.10); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 18px; }
        ...
        .dl-toast { display: none; position: fixed; bottom: 100px; left: 50%; transform: translateX(-50%); background: #1a8fb3; color: #fff; padding: 12px 24px; border-radius: 50px; font-size: 14px; font-weight: 700; z-index: 300; box-shadow: 0 4px 14px rgba(26,143,179,0.35); white-space: nowrap; }
...
<div class="shell">
    ...
    <div class="content">
        ...
        {{-- SEARCH & FILTER --}}
        <form method="GET" action="{{ route('mitra.pengunduhan.index') }}" class="search-wrap">
            ...
            <div class="filter-row">
                ...
                @if(request('search') || request('sort'))
                    <a href="{{ route('mitra.pengunduhan.index') }}" class="filter-clear">✕ Reset</a>
                @endif
            </div>
        </form>
        ...
        @forelse($pengajuanList as $i => $item)
            ...
            <div class="outer-card">
                ...
                <a href="{{ route('mitra.pengunduhan.download', $item->id) }}"
                   class="btn-unduh" onclick="showToast()">
        ...
        @empty
            <div class="empty-state">
                <div class="empty-icon">
                    <svg viewBox="0 0 24 24"><path d="M12 2v13M7 11l5 5 5-5"/><rect x="3" y="18" width="18" height="3" rx="1"/></svg>
                </div>
                <div class="empty-title">
                    {{ request('search') ? 'Tidak ada hasil pencarian.' : 'Belum ada SKPP tersedia' }}
                </div>
                @if(!request('search'))
    ...
    <div class="dl-toast" id="dl-toast">⬇ Mengunduh SKPP...</div>
