    ...
    <style>
        * {
        ...
        .status-btn.invalid-active {
            ...
            color: #fff;
            border-color: #ff7675;
            box-shadow: 0 3px 12px rgba(255, 118, 117, 0.35);
        }
        ...
        <div class="form-card">
            ...
            <form method="POST" action="{{ route('staff.pencatatan.store', $pengajuan->id) }}">
                ...
                <input type="text" name="nama_lengkap"
                    ...
                    value="{{ old('nama_lengkap', $existingCatatan->nama_lengkap ?? '') }}" required />
                @error('nama_lengkap')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror

                <div class="field-label">NIP</div>
                <input type="text" name="nip" class="field-input {{ $errors->has('nip') ? 'is-invalid' : '' }}"
                    placeholder="Masukkan NIP" value="{{ old('nip', $existingCatatan->nip ?? '') }}" required />
                @error('nip')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                ...
                <textarea name="catatan" class="field-input" placeholder="Tambahkan catatan jika ada...">{{ old('catatan', $existingCatatan->catatan ?? '') }}</textarea>
