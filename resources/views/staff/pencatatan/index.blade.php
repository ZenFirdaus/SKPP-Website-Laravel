...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Segoe UI', sans-serif; background: #e8f3f8; display: flex; justify-content: center; }
        ...
        .content { flex: 1; padding: 16px 16px 90px; margin-top: -24px; }
        ...
        .search-input { width: 100%; background: #fff; border: none; border-radius: 50px; padding: 12px 44px 12px 18px; font-size: 14px; color: #333; outline: none; box-shadow: 0 2px 8px rgba(0,0,0,0.08); font-family: inherit; }
        ...
        .filter-clear { padding: 9px 14px; border-radius: 50px; background: #fdecea; color: #e74c3c; font-size: 12px; font-weight: 700; text-decoration: none; box-shadow: 0 2px 6px rgba(0,0,0,0.07); display: flex; align-items: center; }

        /* CARDS */
        .card-item { display: flex; align-items: center; gap: 14px; padding: 18px 16px; border-radius: 22px; margin-bottom: 14px; cursor: pointer; text-decoration: none; transition: transform 0.15s, filter 0.15s; }
        ...
        .card-avatar { width: 58px; height: 58px; border-radius: 16px; background: rgba(255,255,255,0.25); flex-shrink: 0; display: flex; align-items: center; justify-content: center; }
        ...
        .badge { font-size: 11px; font-weight: 700; padding: 5px 11px; border-radius: 50px; white-space: nowrap; flex-shrink: 0; }
        ...
        .nav-plus { width: 50px; height: 50px; background: #1a8fb3; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-top: -20px; box-shadow: 0 4px 14px rgba(26,143,179,0.4); cursor: pointer; transition: background 0.2s, transform 0.15s; text-decoration: none; }
...
<div class="shell">
    ...
    <div class="content">
        ...
        {{-- SEARCH & FILTER --}}
        <form method="GET" action="{{ route('staff.pencatatan.index') }}" class="search-wrap">
            ...
            <div class="filter-row">
                ...
                @if(request('search') || request('sort'))
                    <a href="{{ route('staff.pencatatan.index') }}" class="filter-clear">✕</a>
                @endif
            </div>
        </form>
        ...
        @if(request('search'))
            <div class="hasil-pencarian">
                Hasil pencarian: <strong>"{{ request('search') }}"</strong> — {{ $pengajuanList->count() }} ditemukan
            </div>
        @endif
        ...
        @empty
            <div class="empty-state">
                <div class="icon">📋</div>
                <p>{{ request('search') ? 'Tidak ada hasil pencarian.' : 'Belum ada pengajuan yang masuk.' }}</p>
            </div>
        @endforelse
    </div>
