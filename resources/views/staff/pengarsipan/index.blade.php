    ...
    <style>
        * {
        ...
        .modal-overlay {
            ...
            position: fixed;
            inset: 0;
            background: rgba(26, 143, 179, 0.35);
            z-index: 200;
            align-items: center;
            justify-content: center;
        }
...
<body>
        ...
        <div class="content">
            ...
            @forelse($pengajuanList as $i => $item)
                ...
                <div class="outer-card">
                    ...
                    {{-- Tampilkan info file SKPP dari draft --}}
                    @if ($item->draftSkpp)
                        <div class="file-info">
            ...
            @empty
                <div class="empty-state">
                    ...
                    <p>Belum ada SKPP yang siap diarsipkan.</p>
                </div>
            @endforelse
        </div>
    ...
    <script>
        ...
        async function arsipkan(id, btn) {
            btn.disabled = true;
            btn.innerHTML =
                `<svg viewBox="0 0 24 24" style="width:18px;height:18px;stroke:#fff;fill:none;stroke-width:2"><path d="M3 7h18v13H3z"/><path d="M3 7l3-4h12l3 4"/></svg> Mengarsipkan...`;
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (badge) {
                        badge.textContent = 'Diarsipkan';
                        badge.className = 'badge badge-arsip';
                    }
                    btn.innerHTML =
                        `<svg viewBox="0 0 24 24" style="width:18px;height:18px;stroke:#fff;fill:none;stroke-width:2"><path d="M3 7h18v13H3z"/><path d="M3 7l3-4h12l3 4"/></svg> Sudah Diarsipkan`;
                    document.getElementById('modal-kirim').classList.add('active');
                } else {
                    alert(data.message);
                    btn.disabled = false;
                    btn.innerHTML =
                        `<svg viewBox="0 0 24 24" style="width:18px;height:18px;stroke:#fff;fill:none;stroke-width:2"><path d="M3 7h18v13H3z"/><path d="M3 7l3-4h12l3 4"/><line x1="9" y1="12" x2="15" y2="12"/></svg> Arsipkan`;
                }
            } catch (e) {
        ...
        async function kirimMitra() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    notif.style.cssText =
                        'position:fixed;top:24px;left:50%;transform:translateX(-50%);background:#2ecc71;color:#fff;padding:12px 24px;border-radius:50px;font-weight:700;font-size:14px;z-index:999;';
